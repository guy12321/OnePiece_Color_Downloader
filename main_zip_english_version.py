...
if __name__ == "__main__":
    try:
        ...
        for delta_chapter in range(0, chapters_to_download):
            ...
            newline()
            print(f"Downloading chapter: {actual_chapter}/{final_chapter} ({chapters_downloaded} out of {chapters_to_download}) {completed_download_percentage}%")

            # Itero e scarico ogni pagina sino ad ottenere un codice diverso da 200
            starting_page = 1
            while True:
                actual_page = str(starting_page).zfill(2)
                print(f"Downloading page: {actual_page}")
                ...
                if page_download.status_code == 200:
                    ...
                    print("Failed")
                    print("Writing downloaded images to ZIP...")
                    break
